...
def create_parser():
    ...
    This script is used to generate custom enum-header files using glib-mkenums.
    ...
    --regex "string to replace" "post replace string" parameters.
            ''')
    parser.add_argument('output_file', help='Relative path to output enum header file. (see --out-dir)')
...
def create_enum_header(args, fhead, fprod, ftail, vhead, vprod, vtail):
    try:
...
def write_enum_header_file(args, output):
...
if __name__ == '__main__':
    ...
    output = create_enum_header(args, fhead, fprod, ftail, vhead, vprod, vtail)
    ...
    write_enum_header_file(args, output)
